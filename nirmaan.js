...
function buildAbout(){
  ...
  <section class="sec"><h2 class="st">The Team</h2><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(185px,1fr));gap:1.35rem;max-width:900px;margin:2rem auto 0">${[['SS','Shiwanshu Singh','Founder','#6366F1'],['SG','Shivam Gupta','CTO & AI Lead','#8B5CF6'],['US','Utsav Sharma','Head of Product','#10B981'],['AS','Ashutosh Singh','Design Lead','#F59E0B']].map(([i,n,r,c])=>`<div style="text-align:center;padding:1.4rem"><div style="width:68px;height:68px;border-radius:50%;background:${c};color:#fff;display:flex;align-items:center;justify-content:center;font-family:var(--fh);font-size:1rem;font-weight:700;margin:0 auto .9rem">${i}</div><div style="font-family:var(--fh);font-weight:700;color:var(--t);font-size:.93rem">${n}</div><div style="font-size:.76rem;color:var(--t3);margin-top:.18rem">${r}</div></div>`).join('')}</div></section>
  ${buildFooter()}</div>`;
}
